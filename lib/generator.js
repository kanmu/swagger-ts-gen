        ...
        handlebars_1.default.registerHelper("uniquePropertyRefTypes", function (schema) {
            var pts = function (xs) {
                return Object.values(xs)
                    .filter(function (p) { return p.isRef; })
                    .map(function (p) { return p.type; });
            };
            var types = pts(schema.properties);
            var allOfTypes = schema.allOf.map(function (x) { return (x.isRef ? [x.type] : pts(x.properties)); });
            return Array.from(new Set(types.concat.apply(types, allOfTypes)));
        });
    };
